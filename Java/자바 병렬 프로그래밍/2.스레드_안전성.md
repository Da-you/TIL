...
- OOP의 캡슐화, 데이터 은닉은 스레드 안전한 클래스 작성에 도움이 될 수 있다.
...
- 상대적 시점 또는 JVM이 여러 스레드를 교차해 실해하는 상황에 따라 계산의 정확일이 달라질 때 발생
...
>
> 예를 들어, 애플리케이션 수준의 데이터 저장 공간을 생성하는 부분에 LazyInitRace를 사용후 여러 번 호출할 때 서로 다른 저장 공간 인스턴스를 받으면 저장된 내용을 잃거난 여러 스레드에서 객체 목록을 보려 할 때 일관성이 전혀 없을 수도 있다.
...
```
- 단일 연산 참조 변수(lastNumber, lastFactors)는 스레드에 안전하나 UnsafeCachingFactorizer는 경쟁 조건을 가지고 있어 틀린 결과를 낼 수 있다.
- 스레드 안전성은 여러 스레드에서 수행되는 작업의 타임이, 스케줄링에 따른 교차 실행과 관계없이 불변 조건을 유지해야 한다.
- 위 코드는 인수분해 결과를 곱한 결과가 lastNumber에 캐시된 값과 같아야 한다는 불변 조건이 성립해야 서블릿이 정상 동작함
